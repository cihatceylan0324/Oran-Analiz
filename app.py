...
# Sayfa Ayarları
st.set_page_config(page_title="Superliga Hızlı Arşiv", page_icon="⚽", layout="wide")
...
st.title("⚽ Danimarka Superliga 5 Yıllık Hızlı Maç Arşivi (2021 - 2026)")
st.caption("Bekleme yapmadan, tüm Superliga maçlarını ve istatistiklerini anında Excel'e (CSV) aktar.")
...
# Hızlı Veri Çekme Fonksiyonu (Danimarka Superliga ID: 119)
@st.cache_data(ttl=86400)
def api_danimarka_hizli_getir(api_key, sezon):
    headers = {
        'x-apisports-key': api_key,
        'x-rapidapi-key': api_key
    }
    url_fixtures = f"https://v3.football.api-sports.io/fixtures?league=119&season={sezon}"
    try:
...
st.sidebar.markdown("---")
if st.sidebar.button("⚡ Superliga Hızlı Arşivi Derle", type="primary"):
    hedef_sezonlar = [2026, 2025, 2024, 2023, 2022, 2021] if secim_degeri == "ALL" else [secim_degeri]
    
    tum_toplanan_maclar = []
    with st.spinner("Danimarka Superliga maçları hızla yükleniyor..."):
        for s in hedef_sezonlar:
            m_list = api_danimarka_hizli_getir(API_KEY, s)
            tum_toplanan_maclar.extend(m_list)
            
    st.session_state['danimarka_hizli_arsiv'] = tum_toplanan_maclar
    st.session_state['aktif_secim_adi'] = secilen_sezon_key

# Hafızada maç varsa göster ve Excel İndir butonu sun
if 'danimarka_hizli_arsiv' in st.session_state and st.session_state['danimarka_hizli_arsiv']:
    arsiv = st.session_state['danimarka_hizli_arsiv']
    ...
    if toplam_mac > 0:
        ...
        dosya_adi = f"Superliga_HizliArsiv_{st.session_state['aktif_secim_adi'].replace(' ', '_')}.csv"

        col1, col2 = st.columns([3, 1])
        with col1:
            st.success(f"🎉 Danimarka Superliga için toplam **{toplam_mac} adet maç** başarıyla ve anında derlendi!")
        with col2:
            st.download_button(
        ...
        <div class="summary-card">
            <div style="font-size: 16px; font-weight: bold; color: #81c784; margin-bottom: 6px;">
                📊 Danimarka Superliga — {st.session_state['aktif_secim_adi']} Genel İstatistikler
            </div>
    ...
    st.info("👈 Sol menüden **'TÜM SEZONLAR (2021 - 2026)'** veya istediğin yılı seçip **'Superliga Hızlı Arşivi Derle'** butonuna basarak anında indirebilirsin.")
