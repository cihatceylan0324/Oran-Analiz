...
<h1>Tahmin Paneli</h1>
<div class="sub">Poisson modeli · H2H · Bet365 oran karşılaştırması · Esnek Oranlı Geçmiş Maç Arama</div>

<div class="note">
  Free plan sadece 2022–2024 sezonlarına erişim veriyor. İstediğin orana göre (tek veya çoklu) tarama yapabilirsin.
</div>
...
<div class="controls" style="flex-direction:column;align-items:stretch;">
  <div style="font-size:13px;color:var(--accent2);font-weight:700;">Esnek Tek / Çoklu Oranlı Maç Arama</div>
  <div class="small">
    İstediğin tek bir oranı (örneğin sadece MS1'e 1.69) veya birden fazla oranı doldurabilirsin. Boş bırakılan alanlar aramada filtrelenmez.
  </div>
  <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:end;margin-top:8px;">
    <div class="field"><label>MS1 (Ev) - İsteğe bağlı</label><input type="number" step="0.01" id="oddHomeInput" placeholder="Örn: 1.69" style="width:110px;"></div>
    <div class="field"><label>MS0 (Beraberlik) - İsteğe bağlı</label><input type="number" step="0.01" id="oddDrawInput" placeholder="Örn: 3.50" style="width:110px;"></div>
    <div class="field"><label>MS2 (Deplasman) - İsteğe bağlı</label><input type="number" step="0.01" id="oddAwayInput" placeholder="Örn: 3.20" style="width:110px;"></div>
    ...
    <div class="field"><button onclick="esnekOranTara()">Orana Göre Ara</button></div>
  </div>
...
async function macSec(idx){
  ...
  setStatus("Hazır.");
  renderDetail(fixture, homeStats, awayStats, h2h, odds);
}
...
function renderDetail(fixture, homeStats, awayStats, h2h, oddsData){
  const detail = document.getElementById("detail");
...
async function esnekOranTara(){
  const leagueId = document.getElementById("leagueSelect").value;
  const season = document.getElementById("seasonSelect").value;
  
  const hInput = document.getElementById("oddHomeInput").value;
  const dInput = document.getElementById("oddDrawInput").value;
  const aInput = document.getElementById("oddAwayInput").value;

  const hHedef = hInput ? parseFloat(hInput) : null;
  const dHedef = dInput ? parseFloat(dInput) : null;
  const aHedef = aInput ? parseFloat(aInput) : null;
  ...
  if(hHedef === null && dHedef === null && aHedef === null){
    statusEl.textContent = "Lütfen en az bir oran alanına (MS1, MS0 veya MS2) değer girin.";
  ...
  for(let i = 0; i < ftMatches.length; i++){
    ...
    // Sadece doldurulan inputlar tolerans dahilinde kontrol edilir, boşlar es geçilir.
