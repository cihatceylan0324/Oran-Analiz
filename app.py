...
# Sayfa Ayarları
st.set_page_config(page_title="Premier League Hızlı Arşiv", page_icon="⚽", layout="wide")
...
st.title("⚽ İngiltere Premier League 5 Yıllık Hızlı Maç Arşivi (2021 - 2026)")
st.caption("Bekleme yapmadan, tüm maçları ve istatistikleri anında Excel'e (CSV) aktar.")
...
# Hızlı Veri Çekme Fonksiyonu (Oransın, Saniyeler içinde biter)
@st.cache_data(ttl=86400)
def api_premierleague_hizli_getir(api_key, sezon):
    headers = {
...
st.sidebar.markdown("---")
if st.sidebar.button("⚡ Hızlı Arşivi Derle", type="primary"):
    hedef_sezonlar = [2026, 2025, 2024, 2023, 2022, 2021] if secim_degeri == "ALL" else [secim_degeri]
    
    tum_toplanan_maclar = []
    with st.spinner("Maçlar hızla yükleniyor..."):
        for s in hedef_sezonlar:
            m_list = api_premierleague_hizli_getir(API_KEY, s)
            tum_toplanan_maclar.extend(m_list)
            
    st.session_state['premier_hizli_arsiv'] = tum_toplanan_maclar
    st.session_state['aktif_secim_adi'] = secilen_sezon_key

# Hafızada maç varsa göster ve Excel İndir butonu sun
if 'premier_hizli_arsiv' in st.session_state and st.session_state['premier_hizli_arsiv']:
    arsiv = st.session_state['premier_hizli_arsiv']
    ...
    if toplam_mac > 0:
        ...
        dosya_adi = f"PremierLeague_HizliArsiv_{st.session_state['aktif_secim_adi'].replace(' ', '_')}.csv"

        col1, col2 = st.columns([3, 1])
        with col1:
            st.success(f"🎉 Toplam **{toplam_mac} adet maç** başarıyla ve anında derlendi!")
        with col2:
            st.download_button(
        ...
        <div class="summary-card">
            ...
            <div style="display: flex; gap: 10px; flex-wrap: wrap; font-size: 13px; margin-top: 10px;">
                ...
                <span style="background:#1e2720; color:#a5d617; padding:5px 10px; border-radius:6px; border:1px solid #2e7d32;">2.5 Üst: %{round(ust_sayisi/toplam_mac*100)}</span>
    ...
    st.info("👈 Sol menüden **'TÜM SEZONLAR (2021 - 2026)'** veya istediğin yılı seçip **'Hızlı Arşivi Derle'** butonuna basarak saniyeler içinde indirebilirsin.")
