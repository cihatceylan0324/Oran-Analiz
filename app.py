...
# Sayfa Ayarları
st.set_page_config(page_title="Birebir Oran Analiz Merkezi", page_icon="⚽", layout="wide")
...
st.title("⚽ Birebir Oran Analiz Sistemi (25 Ülke & 6 Yıllık Veri)")
...
def api_mac_ve_oranlari_getir(api_key, lig_id, sezon):
    headers = {
        'x-apisports-key': api_key,
        'x-rapidapi-key': api_key
    }
    
    # 1. Maç Fikstürlerini Çek
    ...
    try:
        ...
        if data_fix.get("errors") and len(data_fix["errors"]) > 0:
            st.error(f"API Hatası: {data_fix['errors']}")
            return []
        ...
        # 2. Oran Verilerini Çek (/odds servisi)
        url_odds = f"https://v3.football.api-sports.io/odds?league={lig_id}&season={sezon}&bookmaker=1"
        ...
        # 3. Fikstür ile Oranları Birleştir
        maclar = []
        for item in fixtures_raw:
            ...
            if f["status"]["short"] in ["FT", "AET", "PEN"]:
                ...
                # Gerçek oranlar varsa al, yoksa varsayılan göster
                f_id = f["id"]
...
secilen_sezon = st.sidebar.selectbox("Sezon / Yıl Seçin", SEZONLAR)
...
yuklenen_maclar = api_mac_ve_oranlari_getir(API_KEY, lig_id, secilen_sezon)
...
if not yuklenen_maclar:
    st.info(f"🔄 {secilen_lig_adi} ({secilen_sezon} Sezonu) verileri çekiliyor...")
else:
    st.success(f"✅ {secilen_lig_adi} ({secilen_sezon} Sezonu) - Toplam {len(yuklenen_maclar)} tamamlanmış maç yüklendi!")

    if not aktif_filtreler:
        st.info("👈 Analiz yapmak için sol menüden oran filtrelerini girebilirsiniz.")
    else:
        esleseler = []
        ...
        if not esleseler:
            st.warning("⚠️ Girilen oran kombinasyonuna uygun maç bulunamadı.")
        else:
